...
def tussleOptions():
    ...
    if num == 2:
        ...
        print60(textOut)
        textOut=textGenSmol("titanium frame!")
        print90(textOut)
        stall()
        resetAll()
        enemyGen(4)
...
def statusReport():
    ...
    textOut=textGen("Press space to continue!")
    screen.blit(textOut,(130,150))
...
def failure():
    ...
    print30(textOut)
    stall()
    webbrowser.open("https://www.youtube.com/watch?v=_8_7Orhey-g")
    time.sleep(1.1)
    pygame.quit()
        ...
        if event.type == pygame.KEYDOWN:
             if event.key == pygame.K_SPACE:
                  if introComplete is not True:
                      if not musicStart:
                           pygame.mixer.music.load('C:/Python34/finalProject/theme.mp3')
